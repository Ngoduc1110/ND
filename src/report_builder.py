...
<body>
    <div class="container">
        <div class="tabs">
            ...
            <a href="industry_report.html">Industry Indices</a>
        </div>
...
def generate_industry_report(volume_chart_html, candlestick_htmls, output_path):
    """Generate the HTML report for Industry Indices with interactive Plotly charts."""
    
    # Combine candlestick divs
    candlesticks_combined = "\n".join([f'<div class="chart-container">{ch}</div>' for ch in candlestick_htmls])
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Chỉ Số Ngành (Industry Indices)</title>
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;600;700&display=swap" rel="stylesheet">
...
<body>
    <div class="container">
        <div class="tabs">
            <a href="index.html">News Dashboard</a>
            <a href="institutional_report.html">Institutional Trading</a>
            <a href="industry_report.html" class="active">Industry Indices</a>
        </div>

        <header>
            <h1>Báo Cáo Chỉ Số Ngành</h1>
            <p style="color: var(--text-dim);">Dữ liệu giao dịch theo nhóm ngành cập nhật tự động</p>
        </header>

        <h2 class="section-title">1. Khối lượng Giao Dịch (Top 5 Ngành Thanh Khoản Nhất)</h2>
        <div class="chart-container">
            {volume_chart_html}
        </div>

        <h2 class="section-title">2. Biến Động Nến (Candlestick) 3 Tháng Gần Nhất</h2>
        {candlesticks_combined}
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        f.write(html)
    print(f"[*] Generated industry report at {output_path}")
